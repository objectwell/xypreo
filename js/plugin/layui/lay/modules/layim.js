...
function gzkyItimeClick (){
    // alert(1)
}
;layui.define(["layer", "laytpl", "upload"], function (i) {
    
    var a = "3.7.5", e = layui.$, t = layui.layer, n = layui.laytpl, l = layui.device(), s = "layui-show",
        o = "layim-this", d = 20, r = {}, c = function () {
            this.v = a, e("body").on("click", "*[layim-event]", function (i) {
                
                var a = e(this), t = a.attr("layim-event");
                if(t == "chat") {
                    var RID = a.attr("class").split("layim-group")[1]
                    if(RID) {
                        enterRoom(mine.id,RID)
                    }
                }
    ...
    var u = function (i) {
        ...
        h = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div onclick="gzkyItimeClick()" class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
    ...
    layui.data.date = function (i) {
        ...
        // 去除html 标签正则删除
        // .replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&quot;")
        return i = (i || "").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(/face\[([^\s\[\]]+?)\]/g, function (i) {
    ...
    var x, b, w, k, C, S = function (i, a, n) {
    ...
    }, D = function () {//这里是PC端聊天界面点击发送按钮后的事件处理
        var i = {
        ...
        // console.log("发送前:"+a.textarea.val());
        var temp_content = a.textarea.val();
        var audio_idx = temp_content.indexOf("audio[");
        var img_idx = temp_content.indexOf("img[");
        var file_idx = temp_content.indexOf("file[");
        var video_idx = temp_content.indexOf("video[");

        if(audio_idx == 0 || img_idx== 0 || file_idx==0 || video_idx==0){//是特殊内容,不做正则处理
        }else{//是普通文本,可以做正则处理
            var reg = /(http:\/\/|https:\/\/)((\w|=|\?|\.|\/|&|-)+)/g;
            temp_content = temp_content.replace(reg, "a($1$2)[$1$2]");
        }
        // console.log("发送后:"+temp_content);

        if (i.content = temp_content, "" !== i.content.replace(/\s/g, "")) {
            if (i.content.length > l) return t.msg("内容最长不能超过" + l + "个字符");
            // 发送消息设置为： 数据提交成功后，后台数据接受渲染
            // e.append(n(h).render(i));   
            // ------------d
            // console.log(e.find(".layim-chat-mine"))
        ...
        status: function (i, a) {
            ...
            layui.upload.render({
                ...
                accept: a,
                    ...
                    // gzky  修改后台返回数据
                    200 == i.code ? (i.data[0] = i.data[0] || {}, "images" === a ? ai(n.textarea[0], "img[" + (i.data[0].url || "") + "]") : "file" === a && ai(n.textarea[0], "file(" + (i.data[0].url || "") + ")[" + (i.data.name || "下载文件") + "]"), D()) : t.msg(i.msg || "上传失败")
                    if(l.cb) {
                        l.cb(i)
                    }
                }
    ...
    i("layim", new c)
}).addcss("modules/layim/layim.css?v=3.7.5", "skinlayimcss");