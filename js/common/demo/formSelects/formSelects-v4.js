...
(function (layui, window, factory) {
	...
	Common.prototype.createDD = function (id, item, clz) {
		...
		var icon ='<i class="xm-iconfont"></i>';
		var pidCl = pid.length==1?"xm-tree-hidn":""
		return '<dd lay-value="' + item.value + '" class="' + (item.disabled ? DISABLED : '') + ' ' + (clz ? clz : '') + '" ' + attr + '>\n\t\t\t\t\t<div class="xm-unselect xm-form-checkbox ' + (item.disabled ? DISABLED : '') + '"  style="margin-left: ' + (pid.length ) * 30 + 'px">\n\t\t\t\t\t\t'+ icon +'\n\t\t\t\t\t\t<span name="' + name + '">' + template + '</span>\n\t\t\t\t\t</div>\n\t\t\t\t</dd>';
	};
